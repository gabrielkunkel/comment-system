...
describe("graphQl comments plus test", () => {
    ...
    it("can add comment", (done) => {
        const query: string = `mutation {
            createOrUpdateComment(_id: "4", author: "Brian", text: "This is my favorite video of all time.") {
              _id
              author
              text
            }
    ...
    it("can adds comment and if no _id is provided, a uuid will be filled in automatically ", (done) => {
        const query: string = `mutation {
            createOrUpdateComment(author: "Brian", text: "This is my favorite video of all time.") {
              _id
              author
              text
            }
    ...
    it("should retrieve all records from database", (done) => {
        ...
        request(app)
            ...
            .end((err, res) => {
                if (err) { return done(err); }
                expect(res.body.data.comments).to.have.lengthOf(arrayOfNewElements.length);
                res.body.data.comments.sort((a: IComment, b: IComment) => parseInt(a._id, 10) - parseInt(b._id, 10));
                expect(res.body.data.comments).to.deep.equal(arrayOfNewElements);
                done();
            });
    });
